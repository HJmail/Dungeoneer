...
public class Inventory {
	...
	/**
	 * Attach a {@link GameView} so the inventory can display messages through the
	 * GUI/console instead of printing directly.
	 *
	 * <p>
	 * If no view is set, messages fall back to System.out.
	 *
	...
	/**
	 * addItem is a method where if the player picks up a Pillar type of - A being
	 * Abstract pillar - E being Encapsulation pillar - I being Inheritance pillar -
	 * P being Polymorphism pillar If the player has a pillar it's true and if the
	 * player doesn't have a pillar it it's false.
	 * 
	 * <p>
	 * Adds an item to the player's inventory. Potions are stackable (maximum 3 per
	 * type). Pillars are tracked individually.
	 * </p>
	 *
	 ...
	 * @return true if the item was actually added/stacked, false if it was rejected
	 *         (full inventory / full stack).
	 */
	public boolean addItem(final Item theItem) {
		...
		if (getNonPillarItemCount() >= MAX_ITEMS) {
			System.out.println("Your inventory is full. You cannot carry more than " + MAX_ITEMS + " items.");
			return false;
		}

		myInventory.add(theItem);
		System.out.println("Picked up item: " + theItem.getClass().getSimpleName());
		return true;
	}

	/**
	 * Uses (consumes or activates) an item by name. Supports both stackable items
	 * (like potions) and non-stackable items.
	 *
	...
	public void useItem(final String theItemName) {
		...
		if (toUse instanceof Potion) {
			...
			} else if (toUse instanceof VisionPotion vp) {

				// ⭐ VISION POTION LOGIC ⭐
				if (myOwner != null) {
					Dungeon dungeon = myOwner.getCurrentDungeon();

					if (dungeon != null && myView != null) {

						// 3×3 visible rooms around hero
						List<Point> visible = VisionHelper.getVisibleRooms(dungeon, myOwner);

						myView.showVisionArea(visible);
						log("Vision Potion used! Revealing nearby rooms.");
					}
				}

			} else {
				// Other future potions
				toUse.use();
			}
		}
	}
	...
	/**
	 * Drops (discards) one instance of an item by its simple class name. For
	 * potions, only one is removed from the stack. For other items the whole item
	 * is removed. Pillars are not dropped.
	 *
	...
	/**
	 * Determines whether the player can exit the dungeon. The player can only exit
	 * if all four pillars (A, E, I, and P) have been collected.
	 * 
	 * @return {@code true} if all pillars are collected; {@code false} otherwise
	 */
	public boolean canExit() {
		return myPillarAbstractionCollected && myPillarEncapsulationCollected && myPillarInheritanceCollected
				&& myPillarPolymorphismCollected;
	}

	/**
	 * Returns how many of the four Pillars of OO have been collected. This method
	 * counts the number of Pillar collection flags currently set to {@code true}.
	...
	/**
	 * Clears all items and resets the inventory to its initial state. This method
	 * removes every item from the inventory list and potion stacks, and resets all
	 * Pillar collection flags to {@code false}.
	 * 
	 * <p>
	 * Intended for use when starting a new game or resetting the dungeon.
	 * </p>
	 */
