...
/**
 * Swing-based GUI for the Dungeoneer game. Shows HUD (gold, health, pillars,
 * inventory), a main game panel, and a text dialog area.
 */
public class DungeoneerFrame extends JFrame implements GameView {
	...
	public DungeoneerFrame(final Dungeon theDungeon, final Hero theHero) {
		...
		int targetSize = 128;
		Image scaled = rawDeadIcon.getImage().getScaledInstance(targetSize, targetSize, Image.SCALE_SMOOTH);
	...
	private void setupControls() {
		...
		myGamePanel.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(final KeyEvent e) {
				...
				case KeyEvent.VK_P -> {
					// Hidden debug, print the whole dungeon map
					String fullMap = myDungeon.debugPrintDungeon();
					showMessage("\n=== DEBUG: FULL DUNGEON MAP ===\n" + fullMap);
					return; // do NOT move the hero
				}
				...
				if (dx != 0 || dy != 0) {
					...
					boolean movedToNewTile = (myGamePanel.getHeroX() != oldX || myGamePanel.getHeroY() != oldY);
					...
					if (movedToNewTile && myGamePanel.isHeroOnPit()) {
						...
						if (newHp <= 0) {
							...
							showMessage(
									myHero.getName() + " fell into a pit and took " + damage + " damage! HP now: 0");
							...
							showMessage(myHero.getName() + " fell into a pit and took " + damage + " damage! HP now: "
									+ myHero.getHitPoints());
						}
					}
					...
					case HEALING_POTION -> {
						boolean added = myHero.getInventory().addItem(new HealingPotion(25, Rarity.COMMON));
					...
					case VISION_POTION -> {
						boolean added = myHero.getInventory().addItem(new VisionPotion(3, Rarity.COMMON));
					...
					case SPEAR, FALCHION, FLAIL, MORNING_STAR, STICK -> {
						...
						if (added) {
							...
							showMessage("Your inventory is full. You leave the " + weapon.getName() + ".");
						}
					}
	...
	/**
	 * Shows a simple inventory popup. For now it just displays the hero's gold;
	 * later we can list real items from the Inventory model.
	 */

	private void showInventoryDialog() {
		InventoryDialog dialog = new InventoryDialog(this, myHero, myHero.getInventory());
	...
	private void handlePlayerDeath() {
		...
		String message = myHero.getName() + " has fallen into a pit and died.\n"
				+ "You will respawn at the entrance or you can exit the dungeon.";

		String[] options = { "Respawn", "Exit" };

		int choice = JOptionPane.showOptionDialog(this, message, "You Died", JOptionPane.DEFAULT_OPTION,
				JOptionPane.INFORMATION_MESSAGE, myDeadIcon, // <-- class-specific dead icon
				options, options[0]);
	...
	private Icon getHudIconForItem(final Item item) {
		...
		if (item instanceof Weapon weapon) {
			...
			if (name.contains("stick"))
				return myHudStickIcon;
			if (name.contains("spear"))
				return myHudSpearIcon;
			if (name.contains("flail"))
				return myHudFlailIcon;
			if (name.contains("falchion"))
				return myHudFalchionIcon;
			if (name.contains("morning star"))
				return myHudMorningStarIcon;
		}
	...
	public void showShopItems(final ArrayList<Item> theItems) {
		...
		JOptionPane.showMessageDialog(this, sb.toString(), "Shopkeeper's Wares", JOptionPane.INFORMATION_MESSAGE);
	}

	@Override
	public int askShop() {
		String input = JOptionPane.showInputDialog(this, "Enter the number of the item you want to buy (or 0 to exit):",
				"Shop", JOptionPane.QUESTION_MESSAGE);
	...
	public void showPit(final int thePitDmg) {
		JOptionPane.showMessageDialog(this, "You fell into a pit and took " + thePitDmg + " damage!", "Pit Damage",
				JOptionPane.WARNING_MESSAGE);
	}

	@Override
	public void showPillar(final char theChar) {
		JOptionPane.showMessageDialog(this, "You found the Pillar of " + pillarName(theChar) + "!", "Pillar Found",
	...
	public void gameOver() {
	    ...
	    // Reveal the full dungeon map before closing
	    String fullMap = myDungeon.debugPrintDungeon();
	    showMessage("\n=== FINAL DUNGEON REVEAL ===\n" + fullMap);

	    JOptionPane.showMessageDialog(
	            this,
	            "Game Over!\nFull dungeon revealed in the log window.",
	            "Game Over",
	            JOptionPane.INFORMATION_MESSAGE
	    );
	...
	public String askCombatChoice(final Hero hero, final Monster monster) {
		...
		int choice = javax.swing.JOptionPane.showOptionDialog(this,
				hero.getName() + " vs " + monster.getName() + "\nChoose your attack:", "Combat Choice",
				javax.swing.JOptionPane.DEFAULT_OPTION, javax.swing.JOptionPane.QUESTION_MESSAGE, null, options,
				options[0]);
	...
	public void showVisionArea(final List<Point> theVisibleRooms) {
		...
		showMessage("Vision Potion active! Revealing " + theVisibleRooms.size() + " nearby rooms.");

		// OPTIONAL: Cristian can later highlight tiles on the board here.
	}
